  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setActiveTab('sales')}>
          ...
          <CardContent className="p-3 sm:p-6 pt-0">
            <div className="text-lg sm:text-2xl font-bold">{formatCurrency(todayRevenue)}</div>
        ...
        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setActiveTab('customers')}>
          ...
          <CardContent className="p-3 sm:p-6 pt-0">
            <div className="text-lg sm:text-2xl font-bold">{formatCurrency(totalDue)}</div>
            <p className="text-xs text-gray-500 mt-1">Outstanding</p>
          </CardContent>
        </Card>

        <Card className={`cursor-pointer hover:shadow-md transition-shadow ${lowStockProducts.length > 0 ? 'border-red-200 bg-red-50' : ''}`} onClick={() => setActiveTab('products')}>
          <CardHeader className="flex flex-row items-center justify-between pb-1 sm:pb-2 p-3 sm:p-6">
            <CardTitle className="text-xs sm:text-sm font-medium text-gray-600">Low Stock</CardTitle>
            <AlertCircle className={`w-4 h-4 sm:w-5 sm:h-5 ${lowStockProducts.length > 0 ? 'text-red-500' : 'text-gray-400'}`} />
          </CardHeader>
          <CardContent className="p-3 sm:p-6 pt-0">
        ...
        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setActiveTab('reports')}>
          ...
          <CardContent className="p-3 sm:p-6 pt-0">
            <div className="text-lg sm:text-2xl font-bold">{formatCurrency(monthlyRevenue - monthlyExpenses)}</div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        <Button onClick={() => setQuickSaleOpen(true)} className="h-16 sm:h-20 text-sm sm:text-lg">
          <ShoppingCart className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2" />
          <span className="hidden sm:inline">New </span>Sale
        </Button>
        <Button variant="outline" onClick={() => setQuickProductOpen(true)} className="h-16 sm:h-20 text-sm sm:text-lg">
          <Package className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2" />
          <span className="hidden sm:inline">Add </span>Product
        </Button>
        <Button variant="secondary" onClick={() => setQuickExpenseOpen(true)} className="h-16 sm:h-20 text-sm sm:text-lg">
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-2">
        ...
        <Button onClick={() => { setEditProduct(null); setFormData({ name: '', price: '', stock: '', category: '' }); setDialogOpen(true); }}>
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map(product => (
                    <TableRow key={product.id}>
                      ...
                      <TableCell className="text-xs hidden sm:table-cell">{product.category || '-'}</TableCell>
                      <TableCell className="text-xs sm:text-sm text-right">{formatCurrency(product.price)}</TableCell>
                      <TableCell className="text-xs sm:text-sm text-right">{product.stock}</TableCell>
                      <TableCell className="hidden sm:table-cell">
                        <Badge variant={product.stock <= 5 ? 'destructive' : product.stock <= 10 ? 'secondary' : 'default'} className="text-xs">
                          {product.stock <= 5 ? 'Low' : product.stock <= 10 ? 'Limited' : 'In Stock'}
                        </Badge>
                      </TableCell>
        ...
        <DialogContent className="max-w-sm sm:max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} placeholder="e.g., Food, Drinks" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleSubmit}>{editProduct ? 'Update' : 'Add'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
              ...
              <Card key={customer.id} className="hover:shadow-md transition-shadow">
                ...
                <CardContent className="p-3 sm:p-4 pt-0">
                  ...
                  <div className="flex gap-1 sm:gap-2">
                    ...
                    {due > 0 && <Button size="sm" className="text-xs" onClick={() => setPaymentDialogOpen(customer.id)}>Pay</Button>}
                  ...
                  filteredSales.map(sale => (
                    <TableRow key={sale.id}>
                      ...
                      <TableCell className="text-xs">
                        <div className="truncate max-w-[120px] sm:max-w-xs">{sale.items.map(i => `${i.productName}(${i.quantity})`).join(', ')}</div>
                      </TableCell>
                      <TableCell className="hidden sm:table-cell">
...
function NewSaleDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const addItem = () => {
    ...
    if (qty <= 0 || qty > product.stock) {
      toast({ title: 'Error', description: 'Invalid quantity or insufficient stock', variant: 'destructive' });
      return;
    }
    ...
    if (existingIndex >= 0) {
      ...
      setItems([...items, { productId: product.id, productName: product.name, quantity: qty, price: product.price, subtotal: product.price * qty }]);
    }
    setSelectedProduct('');
    setQuantity('1');
  };
  ...
  const handleSubmit = () => {
    ...
    addSale({ items, total, paymentType, customerId: paymentType === 'due' ? selectedCustomer : undefined, customerName: paymentType === 'due' ? customer?.name : undefined });
        ...
        <div className="space-y-4">
            ...
            <Select value={selectedProduct} onValueChange={setSelectedProduct}>
              ...
              <SelectContent>
                {products.filter(p => p.stock > 0).map(p => (
                  <SelectItem key={p.id} value={p.id}>{p.name} - {formatCurrency(p.price)} (Stock: {p.stock})</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <div className="flex gap-2">
              <Input type="number" className="w-20" value={quantity} onChange={e => setQuantity(e.target.value)} min="1" />
              <Button onClick={addItem} disabled={!selectedProduct}>Add</Button>
            </div>
          </div>
                ...
                <TableBody>
                  {items.map(item => (
                    <TableRow key={item.productId}>
                      ...
                      <TableCell className="text-xs text-right">{formatCurrency(item.price)}</TableCell>
                      <TableCell className="text-xs text-right">{item.quantity}</TableCell>
                      <TableCell className="text-xs text-right">{formatCurrency(item.subtotal)}</TableCell>
                      <TableCell><Button variant="ghost" size="sm" onClick={() => setItems(items.filter(i => i.productId !== item.productId))}><X className="w-4 h-4" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-stretch sm:items-center">
            ...
            <div className="flex gap-2 flex-1">
              <Button variant={paymentType === 'cash' ? 'default' : 'outline'} onClick={() => setPaymentType('cash')} className="flex-1 sm:flex-none">
                <Wallet className="w-4 h-4 mr-2" />Cash
              </Button>
              <Button variant={paymentType === 'due' ? 'default' : 'outline'} onClick={() => setPaymentType('due')} className="flex-1 sm:flex-none">
          ...
          <div className="text-right text-lg sm:text-xl font-bold">Total: {formatCurrency(total)}</div>
        </div>
        <DialogFooter className="flex-col sm:flex-row gap-2">
          <Button variant="outline" onClick={onClose} className="w-full sm:w-auto">Cancel</Button>
          <Button onClick={handleSubmit} disabled={items.length === 0} className="w-full sm:w-auto">Complete Sale</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
                ...
                {filteredExpenses.length === 0 ? (
                  ...
                  filteredExpenses.map(expense => (
                    <TableRow key={expense.id}>
                      ...
                      <TableCell className="text-xs text-right font-medium text-red-500">-{formatCurrency(expense.amount)}</TableCell>
                      <TableCell className="text-right">
...
function Reports() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        <Card><CardContent className="p-3 sm:p-6"><div className="text-xs text-gray-500">Total Revenue</div><div className="text-base sm:text-2xl font-bold text-green-600">{formatCurrency(totalRevenue)}</div></CardContent></Card>
        <Card><CardContent className="p-3 sm:p-6"><div className="text-xs text-gray-500">Total Expenses</div><div className="text-base sm:text-2xl font-bold text-red-600">{formatCurrency(totalExpenses)}</div></CardContent></Card>
        <Card><CardContent className="p-3 sm:p-6"><div className="text-xs text-gray-500">Net Profit</div><div className={`text-base sm:text-2xl font-bold ${totalRevenue - totalExpenses >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(totalRevenue - totalExpenses)}</div></CardContent></Card>
        <Card><CardContent className="p-3 sm:p-6"><div className="text-xs text-gray-500">Outstanding Dues</div><div className="text-base sm:text-2xl font-bold text-orange-600">{formatCurrency(totalDue)}</div></CardContent></Card>
      </div>
            ...
            {expenseByCategory.length === 0 ? <p className="text-gray-500 text-center py-8 text-sm">No expenses</p> : (
              <ResponsiveContainer width="100%" height={200} className="sm:h-[250px]">
                <RechartsBar data={expenseByCategory}>
                  ...
                  <YAxis tick={{ fontSize: 10 }} width={45} />
                  <Tooltip formatter={(value: number) => formatCurrency(value)} />
                  <Bar dataKey="value">{expenseByCategory.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Bar>
                </RechartsBar>
              </ResponsiveContainer>
            )}
          </CardContent>
...
function QuickProductDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleSubmit = () => {
    ...
    addProduct({ name: formData.name, price: parseFloat(formData.price), stock: parseInt(formData.stock), category: formData.category || 'General' });
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-sm sm:max-w-lg">
        ...
        <div className="space-y-4">
          <div><Label>Name *</Label><Input value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><Label>Price (৳) *</Label><Input type="number" step="0.01" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} /></div>
